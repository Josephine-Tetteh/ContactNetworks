...
def simfxn(Time,popul):
    ...
    for time in range(Time): #no. of days  
        if len(g.vs.select(state_eq = "I"))> int(300):
            cutime = time
            inf_nodes = []
            for inf in g.vs.select(state_eq = "I"):
                inf_nodes.append(g.neighbors(inf))
                for ninf in g.neighbors(inf):
                    g.vs[ninf]['tcount'] = cutime
                    g.vs[ninf]['state'] = 'TB'
           
        for qq in g.vs.select(state_eq='TB'):
            if time == g.vs[qq.index]["tcount"] + 3:
                g.vs[qq.index]["state"] = "V"
        ...
        for m in g.vs.select(state_eq = "I"): #iterates through each node in the network
            g.vs[m.index]["duration"] += 1 #from day 0 this node continues to infect                                
