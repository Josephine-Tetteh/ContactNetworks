...
gfunct <- function(G){
  ...
  return(list(SU=SU,NS=NS,SS=SS,RM=RM,ICU=ICU,HP=HP,MS=MS,Time=Time,R0 = fro[[1]],VAg = VAge,vin = vin))
}
...
dfr = data.frame(c(la1),c(tapply(nr0df$val, nr0df$grp, mean)))
dfr
