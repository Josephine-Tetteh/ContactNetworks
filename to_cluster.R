...
pdf("contdist.pdf")
cdpl=ggplot(mtmx, aes(participants,contacts, fill=nval)) + 
  geom_raster()+
  ...
  for(ti in 1:length(seq(1,21,1))){ 
    ...
    while(length(active)>0){
      ...
      for(v in active){
        ...
        daily_contacts <- Gr$AdjList[[v]]
        E(Gr)[v]$col <-  "#FC4E07"
        V(Gr)[v]$duration <- V(Gr)[v]$duration+1
        for(v1 in daily_contacts){
          new_col <- sample(c( "#FC4E07","#00AFBB"), 1 ,prob<-c(p,1-p))  # stage of infection     
          if(V(Gr)[v1]$col == "#00AFBB" & new_col== "#FC4E07"){ 
            V(Gr)[v1]$col <-  "#FC4E07"  # infected
            V(Gr)[v1]$cnum <- 2
...
Inf_from_net <- compiler::cmpfun(Inf_from_net)
###########################################

source("Inf_from_net.R")
...
R0Ag <- function(G){
  ...
  totpop = length(V(G))
  vin = sample(totpop-1,1,replace = TRUE)
  vin
  VAge = GAge[vin]
  VAg = which(la1==dfGAg$agegrps[vin])
  Glist <- Inf_from_net(G,vin,0.148)
  length(Glist)
...
R0table = replicate(100, R0Ag(G))

write.table(R0table, "R0TABLE.txt")
