...
gfunct <- function(G){
  ...
  ### vaccinate n nodes with highest degree centrality (hubs)
  vacgroup = unlist(sdf[sdf$DC_r<101,7])
  ...
  R0net <- function(net = gr$g, All=FALSE) {
    n  <- igraph::delete.vertices(net,V(tail(Glist,1)[[1]])[vacgroup]) 
